...
ABOUT = "Tuned SPMD+CTD on Grid-World-lowdim from enhanced-v2 tuning"
...
def setup_setting_files(seed_0, n_seeds, n_iters, print_info, skip_save=False):
    ...
    env_name_params_arr = [
        ("gridworld_small_low_dim", 0.9,   "Gaussian", -1, TS, 0.5, 1, 100, -0.5, 1),
        ("gridworld_small_low_dim", 0.99,  "Gaussian", -1, TS, 0.5, 10, 1, 1.0, 0),
        ("gridworld_small_low_dim", 0.995, "Gaussian", -1, TS, 0.5, 10, 1, 1.0, 0),
        #
        ("gridworld_large_low_dim", 0.9,   "Gaussian", -1, TS, 100, 1, 100, -0.5, 1),
        ("gridworld_large_low_dim", 0.99,  "Gaussian", -1, TS, 0.5, 10, 100, 1.0, 1),
        ("gridworld_large_low_dim", 0.995, "Gaussian", -1, TS, 0.5, 10, 100, 1.0, 1),
        # use small_low_dim to tune large_low_dim
        ("gridworld_large_low_dim", 0.9,   "Gaussian", -1, TS, 0.5, 1, 100, -0.5, 1),
        ("gridworld_large_low_dim", 0.99,  "Gaussian", -1, TS, 0.5, 10, 1, 1.0, 0),
        ("gridworld_large_low_dim", 0.995, "Gaussian", -1, TS, 0.5, 10, 1, 1.0, 0),
    ]
    ...
    exp_metadata = ["Exp id", "Env name", "gamma", "feat type", "feat size", "update", "eta", "n_batches", "iota_mult", "uLam_mult", "burn_in"]
    row_format ="{:>10}|{:>15}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}"
    if not skip_save:
        print("")
        print(row_format.format(*exp_metadata))
        print("-" * (115+len(exp_metadata)-1))

    ct = 0
    for (eta, (env_name, gamma, feat_type, feat_size, update, eta, n_batches, iota_mult, uLam_mult, burn_in)) in enumerate(
            env_name_params_arr,
    ):
        ...
        if not skip_save:
            print(row_format.format(ct, od["env_name"], od["gamma"], 
                ...
                od["eta"], od["n_batches"], od["ctd_iota_mult"], 
                od["ctd_uLam_mult"], od["ctd_burn_in"])
            )
