...
DATE = "2025_12_11"
EXP_ID  = 1
ABOUT = "Tuning Auto-explore Monte Carlo lengths based on True or Estimated cost" 
...
def setup_setting_files(seed_0, n_seeds, n_iters, print_info, skip_save=False):
    ...
    od["tune_exploration"] = True
    od["min_T"] = 2e2
    od["max_T"] = 4e5
    od["tune_true_cost"] = False
    ...
    exp_metadata = ["Exp id", "n_iters", "estimator", "T_mc", "true_cost"]
    ...
    for ((n_iters, estimator, T, true_cost),) in itertools.product(n_iters_estimator_T_truecost_arr):
        ...
        print(row_format.format(ct, od["n_iters"], od["estimate_Q"], od["T_mc"], od["tune_true_cost"]))
