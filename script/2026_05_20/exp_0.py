...
ABOUT = "Tune SPMD+CTD on GridWorld with burn-in."
...
def setup_setting_files(seed_0, n_seeds, n_iters, print_info, skip_save=False):
    ...
    exp_metadata = ["Exp id", "Env name", "gamma", "feat frac", "update", "eta", "iota", "ukappa"]
    row_format ="{:>10}|{:>15}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}|{:>10}"
    if not skip_save:
        print("")
    ...
    ct = 0
    for (env_name, gamma, update, eta, iota, ukappa) in itertools.product(
            env_name_arr, gamma_arr, update_rule_arr,
            eta_arr, ctd_iota_arr, ukappa_arr,
    ):
