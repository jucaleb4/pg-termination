...
ABOUT = "Full run of SPMD+CTD-estimate on GARNET (includes estimate and reset)"
...
def setup_setting_files(seed_0, n_seeds, n_iters, print_info, skip_save=False):
    ...
    env_name_params_arr = [
        ("garnet_1000", 0.9, 'ctd', 1e0, 2e3, -0.5),
        ("garnet_1000", 0.99, 'ctd', 1e2, 2e3, -0.5),
        ("garnet_1000", 0.995, 'ctd', 1e2, 2e3, -0.5),
        #
        ("garnet_5000", 0.9, 'ctd', 1e0, 2e3, -0.5),
        ("garnet_5000", 0.99, 'ctd', 1e2, 2e3, -0.5),
        ("garnet_5000", 0.995, 'ctd', 1e2, 2e3, -0.5),
        #
        ("garnet_1000", 0.9, 'ctd_estimate', 1e0, 2e3, -0.5),
        ("garnet_1000", 0.99, 'ctd_estimate', 1e2, 2e3, -0.5),
        ("garnet_1000", 0.995, 'ctd_estimate', 1e2, 2e3, -0.5),
        #
        ("garnet_5000", 0.9, 'ctd_estimate', 1e0, 2e3, -0.5),
        ("garnet_5000", 0.99, 'ctd_estimate', 1e2, 2e3, -0.5),
        ("garnet_5000", 0.995, 'ctd_estimate', 1e2, 2e3, -0.5),
    ]
    ...
    exp_metadata = ["Exp id", "Env name", "gamma", "estimate_Q", "eta", "iota_mult", "uLam_mult"]
    row_format ="{:>10}|{:>15}|{:>10}|{:>15}|{:>10}|{:>10}|{:>10}"
    if not skip_save:
        print("")
    ...
    ct = 0
    for (env_name, gamma, estimate_Q, eta, iota_mult, uLam_mult) in env_name_params_arr:
        ...
        if not skip_save:
            print(row_format.format(ct, od["env_name"], od["gamma"], 
                od["estimate_Q"], od["eta"], od["ctd_iota_mult"], 
                od["ctd_uLam_mult"],
            ))
