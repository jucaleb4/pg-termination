...
ABOUT = "Full run of SPMD+CTD-estimate on GARNET with refined tuning-v3"
...
def setup_setting_files(seed_0, n_seeds, n_iters, print_info, skip_save=False):
    ...
    env_name_params_arr = [
        ("garnet_50", 0.9, -1, 1e0, 1e4, -0.5),
        ("garnet_50", 0.99, 1000, 1e4, 1e2, 1),
        ("garnet_50", 0.995, -1, 1e2, 1e4, -0.5),
        #
        ("garnet_200", 0.9, -1, 1e2, 1e2, -0.5),
        ("garnet_200", 0.99, -1, 1e2, 1e4, -0.5),
        ("garnet_200", 0.995, -1, 1e2, 1e4, -0.5),
    ]
    ...
    exp_metadata = ["Exp id", "Env name", "gamma", "feat_size", "eta", "iota_mult", "uLam_mult"]
    ...
    ct = 0
    for (env_name, gamma, feat_size, eta, iota_mult, uLam_mult) in env_name_params_arr:
