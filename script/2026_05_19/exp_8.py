...
ABOUT = "Full run SPMD-Dyn on GridWorld from Refined tuning-v1 (exp_6.py)"
...
def setup_setting_files(seed_0, n_seeds, n_iters, print_info, skip_save=False):
    ...
    # tuned parameters
    env_name_params_arr = [
        ("gridworld_tiny", 0.9, 0.5), 
        ("gridworld_tiny", 0.99, 0.5), 
        ("gridworld_tiny", 0.995, 0.5), 
        #
        ("gridworld_small", 0.9, 0.5), 
        ("gridworld_small", 0.99, 0.5), 
        ("gridworld_small", 0.995, 0.5), 
        #
        ("gridworld_large", 0.9, 0.5), 
        ("gridworld_large", 0.99, 5.0), 
        ("gridworld_large", 0.995, 5.0), 
    ]
    ...
    row_format ="{:>10}|{:>15}|{:>10}|{:>10}"
    if not skip_save:
        print("")
        print(row_format.format(*exp_metadata))
        print("-" * (45+len(exp_metadata)-1))

    ct = 0
    for (env_name, gamma, eta) in env_name_params_arr:
